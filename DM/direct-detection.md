...
Without `--dm-limit-table`, the existing `lz2025-source` fit is retained, including
its previous low-mass treatment. This fit's name refers to the inherited steering
code, not to a verified implementation of arXiv:2609.02823.
...
permissions. The release appears not to be public yet. No official numerical
table from this release has been imported or activated.
...
metadata. It does not replace the default fit.
...
Once the release has been checked and converted to per-nucleon cross sections,
...
log(cross section). It rejects requested mass ranges outside the table before
creating output; there is no extrapolation or fallback to another experiment.
It preserves the existing abundance fraction xi = min(1, Omega/0.121): the SI
cross-section limit is divided by xi, with an infinite limit for xi = 0.
Relic-density and Fermi-LAT gamma-line checks are unchanged.

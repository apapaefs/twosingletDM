...
p=argparse.ArgumentParser();p.add_argument('--bsmpt-upstream',type=Path,required=True);p.add_argument('--higgstools-upstream',type=Path,required=True);p.add_argument('--archive-dir',type=Path);a=p.parse_args()
...
payload={'physics_version':PHYSICS_VERSION,'release_source_commit':git_revision(ROOT),'host':platform.node(),'platform':platform.platform(),
         ...
         'higgstools_extensions':{name:sha256(importlib.util.find_spec('Higgs.'+name).origin) for name in ('_Higgs','predictions','bounds','signals')},'sm_inputs':sm_inputs(),
         'micromegas_archives':{str(v):sha256((a.archive_dir or root)/f'micromegas_{v}.tgz') or sha256(ROOT/f'DM/micromegas_{v}.tgz') for v in ('7.1.4','6.1.15')},'installed':installed}
